...
interface BrandingListResponse {
...
function BrandingListContent() {
  ...
  const fetchThemes = useCallback(async (showLoading = true) => {
    ...
    setError(null);
    try {
      const res = await apiFetch<BrandingListResponse>(`${ENT_API}/branding`);
      setThemes(res.themes ?? []);
    } catch (err: unknown) {
      const message = sanitizeErrorMessage(err, 'Failed to load themes');
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  const renderTheme = ({ item, index }: { item: BrandingTheme; index: number }) => {
    return (
      <FadeSlideIn delay={index * 50}>
        <TouchableOpacity
          style={styles.card}
          activeOpacity={0.7}
          onPress={() => router.push(`/manage/branding-detail?id=${item.id}` as any)}
        >
          <View style={styles.cardBody}>
            ...
            <View style={styles.colorRow}>
              ...
              <ColorDot color={item.secondary_color} />
              <ColorDot color={item.accent_color} />
              {!item.primary_color && !item.secondary_color && !item.accent_color && (
                <Text style={styles.noColorsText}>No colors configured</Text>
              )}
            </View>
          </View>
  ...
  return (
    <View style={styles.container}>
          ...
          {[0, 1, 2, 3].map((i) => (
            <View key={i} style={styles.skeletonCard}>
              ...
              <View style={{ flexDirection: 'row', gap: SPACING.sm }}>
                <View style={{ backgroundColor: COLORS.border, width: 24, height: 24, borderRadius: 12 }} />
                <View style={{ backgroundColor: COLORS.border, width: 24, height: 24, borderRadius: 12 }} />
                <View style={{ backgroundColor: COLORS.border, width: 24, height: 24, borderRadius: 12 }} />
              </View>
            </View>
          ...
          ListEmptyComponent={
            <View style={styles.centerContainer}>
              <Ionicons
                ...
                style={{ marginBottom: SPACING.lg }}
              />
              <Text style={styles.emptyText}>No branding themes</Text>
              <Text style={styles.emptySubtext}>
                Create a theme to customize your white-label appearance
              </Text>
            </View>
          }
        />
      ...
      <TouchableOpacity
        style={styles.fab}
        activeOpacity={0.8}
        onPress={() => router.push('/manage/branding-create' as any)}
      >
...
export default function BrandingScreen() {
  return (
    <>
      <Stack.Screen options={{ title: 'Branding', headerBackTitle: 'Manage' }} />
